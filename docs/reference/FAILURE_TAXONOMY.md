...
| **FAIL-16** | **Bad Domain Assumption in Non-Protected Path** | **TYPE C** | Agent assumes card flip logic is in an unverified path instead of checking wayfinding indices. | **Skill / Subagent Reasoning**: Handled by Reconnaissance phase in skills (`antios-engineer`) and compiler feedback. |
...
| **FAIL-19** | **Hook Bypass via Shell Redirection** (`run_command`) | **TYPE E** | Agent uses PowerShell/bash (`Set-Content`, `echo > file`, `git apply`) to modify protected files. | **Platform Limitation**: Antigravity hooks intercept tool calls, not arbitrary sub-processes spawned inside shell commands. True boundary protection requires OS-level file permissions (read-only filesystem mount) or containerization. |
