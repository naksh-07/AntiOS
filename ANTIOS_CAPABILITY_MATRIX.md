...
| **7** | Fail-Closed Path Guard Engine | **CORE** | `framework/core/guard.py` | Universal path canonicalization, ancestor isolation, self-protection, and 8.3 alias blocking. 18/18 tests passing. |
...
| **5. Hooks / Enforcement** | **KEEP** | Dual-path `.agents/hooks.json` driving fail-closed Python hooks (`guard.py`, `gate.py`) is proven and tested (18/18 tests pass). | Retain as the primary deterministic enforcement foundation. |
