...
test execution, working tree cleanliness, unresolved conflict marker detection,
...
def discover_test_runners(repo_root: str, config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 1. AntiOS source self-test harness
    ...
    # 3. Python (pyproject.toml, pytest.ini, tests/)
...
def run_command_safe(
    ...
    try:
        ...
        return proc.returncode, stdout, stderr, missing
    except subprocess.TimeoutExpired:
        return -1, "", f"Command timed out after {timeout} seconds", False
    except FileNotFoundError:
        return 127, "", f"Executable '{cmd[0]}' not found", True
...
def evaluate_stop_gate(input_data: Any) -> Tuple[str, Optional[str], Optional[str], Optional[str]]:
    ...
    if not canonical_workspaces:
        discovered = path_resolver.discover_project_root(cwd=os.getcwd(), script_file=__file__)
    ...
    for repo_root in canonical_workspaces:
        # 1. Check working tree conflict markers
        ...
        for runner in test_runners:
            ...
            ret, stdout, stderr, is_missing = run_command_safe(cmd, cwd=repo_root, timeout=timeout_sec)

            if is_missing:
                if is_required:
                    return (
                        "continue",
                        f"AntiOS Stop Gate [MISSING_TEST_RUNNER]: Required test runner '{runner_name}' "
